...
class TimeLineItem extends StatelessWidget {
  ...
  const TimeLineItem(this.logs, {this.comparisonLog, Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    for (var entry in logs){
      TimeLineEntry? comp = comparisonLog?.firstWhereOrNull((element) => element.option == entry.option);
            ...
            child: Container(
              ...
              child: Center(
                child: entry.advChange != entry.adv
                      ? Text(entry.advChange > 0 ? "+${entry.advChange.toString()}" : entry.advChange.toString())
                      :null,
              )
            ),
